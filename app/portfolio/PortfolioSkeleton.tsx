export default function PortfolioSkeleton() {
  return (
    <div className="min-h-screen bg-[var(--color-trove-page-bg)] flex flex-col md:flex-row w-full">
      ...
      {/* 3. Main Portfolio Content Skeleton */}
          ...
          <div className="bg-[var(--color-trove-card-surface)] border border-[var(--color-trove-border)] rounded-[var(--radius-trove-card)] p-6 shadow-sm">
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="h-10 w-full sm:w-64 bg-[var(--color-trove-bg-default)] border border-[var(--color-trove-border)] rounded-lg animate-pulse"></div>
              </div>
              
              {/* Filter Pills Skeleton */}
              <div className="flex flex-wrap gap-2 mt-2">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="h-8 w-20 bg-[var(--color-trove-bg-default)] rounded-full animate-pulse"></div>
                ))}
              </div>
                ...
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="bg-[var(--color-trove-card-surface)] border border-[var(--color-trove-border)] rounded-[var(--radius-trove-card)] p-4 flex items-center justify-between shadow-sm">
                    ...
                    <div className="flex items-center gap-4 w-1/3">
                      <div className="w-12 h-12 rounded-lg bg-[var(--color-trove-bg-default)] border border-[var(--color-trove-border)] shrink-0 animate-pulse"></div>
